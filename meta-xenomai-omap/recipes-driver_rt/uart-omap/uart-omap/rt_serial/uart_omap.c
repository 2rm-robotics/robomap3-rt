...
#define DEFAULT_BAUD_BASE	3000000
...
//core CM_FCLKEN_CORE and CM_ICLKEN_CORE bits
#define EN_UART1		0x2000
#define EN_UART2		0x4000
#define EN_UART3		0x0800
//per cm register offset
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("jan.kiszka@web.de; Guillaume Sanahuja");
					...
					uint64_t * timestamp)
{
	...
	do {
		c = rt_16550_reg_in(base, RHR);	/* read input char */
...
/*	if (uart->i_count >= uart->config.rts_hiwm &&
		...
		uart->modem &= ~MCR_RTS;
		rt_16550_reg_out(base, MCR, uart->modem);
	}*/
...
static inline void rt_16550_stat_interrupt(struct rt_16550_context *ctx)
{
	...
	ctx->status |= (rt_16550_reg_in(base, LSR) &
...
//rtdm_printk("rt_16550_interrupt\n");
	...
	while (1) {
		iir = rt_16550_reg_in(base, IIR) & IIR_MASK;
		...
		if (iir == IIR_RX) {
			...
			modem = rt_16550_reg_in(base, MSR);
			if (modem & (modem << 4))
	...
	rt_16550_reg_out(base, IER, ctx->ier_status);
...
//rtdm_printk("rt_16550_set_config\n");
	...
	if (testbits(config->config_mask, RTSER_SET_BAUD)) {
		...
		rt_16550_reg_out(base, DLM, baud_div >> 8);		
	}
	...
	if (testbits(config->config_mask, RTSER_SET_HANDSHAKE)) {
		...
		rt_16550_reg_out(base, MCR, ctx->mcr_status);
...
//rtdm_printk("rt_16550_open\n");
	...
	if (err) {
		/* reset DTR and RTS */
		rt_16550_reg_out(ctx->base_addr,
				 MCR, 0);
	...
	rt_16550_reg_out(ctx->base_addr, IER,
			 IER_RX);
		   ...
		   unsigned int request, void *arg)
{
	...
	case RTSER_RTIOC_GET_STATUS: {
		...
		if (user_info) {
			...
			status_buf.line_status =
			    rt_16550_reg_in(base, LSR) | status;
	...
	case RTSER_RTIOC_SET_CONTROL: {
		...
		rt_16550_reg_out(base, MCR, new_mcr);
		rtdm_lock_put_irqrestore(&ctx->lock, lock_ctx);
		break;
	}
...
//rtdm_printk("rt_16550_init\n");
	...
	for (i = 0; i < MAX_DEVICES; i++) {
		...
		snprintf(dev->device_name, RTDM_MAX_DEVNAME_LEN, "rtser%d",port_num[i]);
		dev->device_id = port_num[i]-1;

		dev->proc_name = dev->device_name;

		err = rt_16550_init_io(dev->device_id, dev->device_name);
		if (err)
			goto kfree_out;

		if (baud_base[dev->device_id] == 0)
			baud_base[dev->device_id] = DEFAULT_BAUD_BASE;

		if (tx_fifo[dev->device_id] == 0)
			tx_fifo[dev->device_id] = DEFAULT_TX_FIFO;
		...
		if(port_num[i]==1)
		{
			...
			writel(val|EN_UART1,(void *)base + CM_FCLKEN1_CORE);
			val=readl((void *)base + CM_ICLKEN1_CORE);
			writel(val|EN_UART1,(void *)base + CM_ICLKEN1_CORE);
		}
		if(port_num[i]==2)
		{
			base=(unsigned long)ioremap(CORE_CM, 8);
			val=readl((void *)base + CM_FCLKEN1_CORE);
			writel(val|EN_UART2,(void *)base + CM_FCLKEN1_CORE);
			val=readl((void *)base + CM_ICLKEN1_CORE);
			writel(val|EN_UART2,(void *)base + CM_ICLKEN1_CORE);
		}
		if(port_num[i]==3)
		{
			base=(unsigned long)ioremap(PER_CM, 8);
			val=readl((void *)base + CM_FCLKEN_PER);
			writel(val|EN_UART3,(void *)base + CM_FCLKEN_PER);
			val=readl((void *)base + CM_ICLKEN_PER);
			writel(val|EN_UART3,(void *)base + CM_ICLKEN_PER);
		}
		

		//reset
		base = rt_16550_base_addr(dev->device_id);
		rt_16550_reg_out(base, SYSC, SRST);				
		...
		rt_16550_reg_out(base, MDR1, 0x00);
		rt_16550_reg_out(base, SYSC, (0x02 << 3) | (1 << 2) | (1 << 0));		
...
//rtdm_printk("fin rt_16550_init\n");
	return 0;
      ...
      release_io_out:
	rt_16550_release_io(dev->device_id);
