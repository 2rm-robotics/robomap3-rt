...
OECMAKE_CXX_LINK_FLAGS_genericx86-64 ?= "${HOST_CC_ARCH} --sysroot=${SDKTARGETSYSROOT} -Wl,--dynamic-linker=${SDKPATHNATIVE}/lib/ld-linux-x86-64.so.2 -Wl,-rpath=\${LIBGL_PATH}:${SDKTARGETSYSROOT}/usr/lib:${SDKTARGETSYSROOT}/lib ${BUILD_OPTIMIZATION} ${LDFLAGS}"
OECMAKE_CXX_LINK_FLAGS_genericx86 ?= "${HOST_CC_ARCH} --sysroot=${SDKTARGETSYSROOT} -Wl,--dynamic-linker=${SDKPATHNATIVE}/lib/ld-linux.so.2 -Wl,-rpath=\${LIBGL_PATH}:${SDKTARGETSYSROOT}/usr/lib:${SDKTARGETSYSROOT}/lib ${BUILD_OPTIMIZATION} ${LDFLAGS}"
