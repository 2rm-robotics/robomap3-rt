...
OECMAKE_CXX_LINK_FLAGS_genericx86-64 ?= "${HOST_CC_ARCH} --sysroot=${SDKTARGETSYSROOT} -Wl,--dynamic-linker=${SDKPATHNATIVE}/lib/ld-linux-x86-64.so.2 -Wl,-rpath=${SDKTARGETSYSROOT}/usr/lib:${SDKTARGETSYSROOT}/lib:\${LIB_RPATHS} -L${SDKTARGETSYSROOT}/usr/lib/GL ${BUILD_OPTIMIZATION} ${LDFLAGS}"
OECMAKE_CXX_LINK_FLAGS_genericx86 ?= "${HOST_CC_ARCH} --sysroot=${SDKTARGETSYSROOT} -Wl,--dynamic-linker=${SDKPATHNATIVE}/lib/ld-linux.so.2 -Wl,-rpath=${SDKTARGETSYSROOT}/usr/lib:${SDKTARGETSYSROOT}/lib:\${LIB_RPATHS} -L${SDKTARGETSYSROOT}/usr/lib/GL ${BUILD_OPTIMIZATION} ${LDFLAGS}"
...
set( CMAKE_SYSTEM_PROCESSOR_DEFAULTTUNE ${DEFAULTTUNE} )
...
#get system paths for GL libs
execute_process(COMMAND bash "-c" "ldconfig -v 2>/dev/null| grep -v ^$'\t'  | cut -d: -f1" OUTPUT_VARIABLE LIB_RPATHS)
string(REPLACE "\n" ":" LIB_RPATHS \${LIB_RPATHS}) #replace CR by :
